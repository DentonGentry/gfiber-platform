...
void diagMocaStrLog(char *pLogMsg, diag_moca_status_t *pStatus)
{
  ...
  diag_moca_perf_status_entry_t *pPerfResult;
  int i, j, nodes;
  ...
  switch (msgType) {
    case DIAG_MOCA_LOG_NONE:
      ...
      DIAGD_LOG_W_TS(" msgType= %s\n", diagMocaMsgTypeStr[msgType]);
      break;
    ...
    case DIAG_MOCA_LOG_EXCESSIVE_TX_RX_DISCARD_PKTS:
      ...
      DIAGD_LOG_W_TS(" msgType= %s", diagMocaMsgTypeStr[msgType]);
      diagMocaMyStatusLog(true, pStatus);

      DIAGD_LOG_WO_TS(true, "##########Previous  Stats###########");
      diagMocaStatsLog(true, pPrevStats);
      DIAGD_LOG_WO_TS(true, "##########Current  Stats###########");
      diagMocaStatsLog(true, pCurrStats);
      diagMocaNodeStatsLog(true, pNodeStats);
      break;
    case DIAG_MOCA_LOG_POOR_PHY_RATE:
      pPerfStatus = (diag_moca_perf_status_t *)pLogMsg;
      DIAGD_LOG_W_TS(" msgType= DIAG_MOCA_LOG_POOR_PHY_RATE");
      diagMocaMyStatusLog(true, pStatus);
      DIAGD_LOG_WO_TS(true, "noConnectedNodes = %d", pPerfStatus->noConnectedNodes);
      ...
      for (i=0; i < nodes; i++) {
        if (pPerfResult->valid) {
          diagMocaPerfStatusLog(true, pPerfResult);

          for (j=0; j < nodes; j++) {
            if (pPerfResult->nodeId == pPerfStatus->nodeStatus.nodeStatus[j].nodeId) {
              diagMocaNodeStatusLog(true, &pPerfStatus->nodeStatus.nodeStatus[j]);
              break;
            }
          }
        }
      ...
      DIAGD_LOG_WO_TS(true, "###############################");
      break;
    default:
      /* Shouldn't happen */
      DIAGD_TRACE("%s: Invalid MsgType= %d", __func__, msgType);
      DIAGD_LOG_W_TS(" Invalid msgType= %d", msgType);
      break;
  } /* switch (msgType) */
